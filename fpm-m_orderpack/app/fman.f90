...
integer                      :: start_keep, end_keep
    ...
    if(regex.ne.' ')then
       if (getpat(merge(lower(regex),regex,ignorecase), p%pat) .eq. ERR) then
          stop '*fpm-orderpack* Illegal pattern.'
       endif
    endif
    ...
    if(size(manual).eq.0)then
       ...
       write(*,'(g0)')'   fpm-orderpack -e TOPIC'
       write(*,'(g0)')'or search the entire manual:'
       write(*,'(g0)')'   fpm-orderpack manual -i -e TOPIC'
       stop 1
    else
...
integer :: ii,jj,kk
   ...
   if(demo)then
    ...
    if(size(newsection).eq.0)then
       write(*,*)'!<ERROR> *fpm-orderpack* standard demo code format not found for ',trim(topics(i))
       section=['']
    else
       section=newsection
...
'NAME                                                                            ',&
'    fpm-orderpack(1f) - [DEVELOPER] output descriptions of Fortran intrinsics   ',&
'    (LICENSE:PD)                                                                ',&
'                                                                                ',&
'SYNOPSIS                                                                        ',&
'    fpm-orderpack NAME(s) [[-ignorecase][--regex Regular_Expression]]|          ',&
'                [-topic_only][--color][--demo]                                  ',&
'                                                                                ',&
'    fpm-orderpack [ --help| --version]                                          ',&
'                                                                                ',&
'DESCRIPTION                                                                     ',&
'   fpm-orderpack(1) print descriptions of procedures as simple flat text.       ',&
'                                                                                ',&
...
'  Sample commands                                                               ',&
'                                                                                ',&
'   fpm-orderpack              # list table of contents                          ',&
'   fpm-orderpack -e character # check TOC for string. try "loc","prank","sort"  ',&
'   fpm-orderpack tan|less     # display a description of tan(3f)                ',&
'                                                                                ',&
'   fpm-orderpack --regex ''character'' # look for string in TOC ignoring case   ',&
'                                                                                ',&
'   fpm-orderpack manual>fortran.txt    # create a copy of all descriptions      ',&
'                                                                                ',&
...
'   fpm-orderpack -e scan -i manual                                              ',&
'                                                                                ',&
'   fpm-orderpack -d pack >demo_pack.f90 # get sample program to try VERIFY(3f). ',&
'']

version_text=[ CHARACTER(LEN=128) :: &
'PRODUCT:         GPF (General Purpose Fortran) utilities and examples           ',&
'PROGRAM:         fpm-orderpack(1)                                               ',&
